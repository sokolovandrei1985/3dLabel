...
export function switchScene(to: 'perspective' | 'ortho', scn: THREE.Scene = scene): void {
  if (to == 'perspective') {
    const roomG = scn.getObjectByName('roomGroup')
    if (roomG) roomG.visible = false
    const tableG = scn.getObjectByName('tableGroup')
    if (tableG) tableG.visible = true
    //scn.remove(roomGroup)
    //scn.add(tableGroup)
  }
  else {
    const roomG = scn.getObjectByName('roomGroup')
    if (roomG) roomG.visible = true
    const tableG = scn.getObjectByName('tableGroup')
    if (tableG) tableG.visible = false
    //scn.add(roomGroup)
    //scn.remove(tableGroup)
  }
}
...
export function setModelView(model: THREE.Group | THREE.Object3D, view: 'front' | 'back' | 'right' | 'left' | 'top'): void {
  if (!model) return
...
export async function loadTexture(url: string, isEXR: boolean = true): Promise<THREE.DataTexture> {
